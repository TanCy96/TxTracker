...
class PermissiveExtractorTest {
    ...
    @Test
    fun captures_gbp_amount_from_allowlisted_package() {
        // Wise package is in the allowlist. PermissiveExtractor doesn't try to
        // extract a merchant — it stamps `<Source> (review)` and captures the amount.
        val r = extractor.extract(
            text = "Welcome notice: 100 GBP available",
            sourceApp = SourcePackages.GOOGLE_PAY,  // any allowlisted package
            postedAt = now,
        )!!
    ...
    @Test
    fun integer_amount_no_decimals_captured() {
        val r = extractor.extract(
            text = "1 MYR balance",
            sourceApp = SourcePackages.GOOGLE_PAY,
            postedAt = now,
        )!!
    ...
    @Test
    fun symbol_prefix_with_user_default_resolves() {
        val r = extractor.extract(
            text = "spent $5.00 yesterday",
            sourceApp = SourcePackages.GOOGLE_PAY,
            postedAt = now,
            symbolDefaults = mapOf("$" to "SGD"),
        )!!
        assertThat(r.currency).isEqualTo("SGD")
        assertThat(r.amountMinor).isEqualTo(500L)
    }

    @Test
    fun bypass_allowlist_still_works() {
        // Unknown package, but the listener may pass bypass=true (user has
        // capture-all-packages on). Capture proceeds.
        val r = extractor.extract(
            text = "Pay 25 USD now",
            sourceApp = "com.example.unknown",
            postedAt = now,
