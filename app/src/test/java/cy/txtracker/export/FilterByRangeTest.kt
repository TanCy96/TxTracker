...
class FilterByRangeTest {
    ...
    private fun deposit(occurredAt: Instant) = SlDebitDeposit(
        amountMinor = 50_000,
        occurredAt = occurredAt,
        createdAt = occurredAt,
    )
    ...
    @Test
    fun deposit_in_range_kept_and_at_next_malaysia_midnight_dropped() {
        val inRange = deposit(Instant.parse("2026-01-15T04:00:00Z"))        // mid-January MYT
        val atEndExclusive = deposit(Instant.parse("2026-01-31T16:00:00Z")) // 2026-02-01 00:00 MYT
        assertThat(filterDepositsByRange(listOf(inRange, atEndExclusive), january))
            .containsExactly(inRange)
    }

    @Test
    fun deposit_null_range_returns_input_unchanged() {
        val deposits = listOf(deposit(Instant.parse("2020-06-15T00:00:00Z")))
        assertThat(filterDepositsByRange(deposits, null)).isEqualTo(deposits)
    }
}
